...
Client.prototype.addUserProperties = function(identity, properties, callback) {
  options = {
    ...
    json: {
      app_id: validations.normalizeAppId(this.appId),
      identity: validations.normalizeIdentity(identity),
      properties: validations.normalizeProperties(properties),
    },
  };
...
Client.prototype.track = function(eventName, identity, properties, callback) {
  options = {
    ...
    json: {
      app_id: validations.normalizeAppId(this.appId),
      event: validations.normalizeEventName(eventName),
      identity: validations.normalizeIdentity(identity),
    },
  };
  if (typeof(properties) === "function")
    ...
    options.json.properties = validations.normalizeProperties(properties);
  ...
  return new Promise(function(resolve, reject) {
    request(options, function (error, response) {
      if (!error && response.statusCode >= 400)
        error = Error("Heap API server error " + response.statusCode);
